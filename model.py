...
class DualNet(object):
    def __init__(self, sess, image_size=256, batch_size=1,fcn_filter_dim = 64,  \
                 ...
                 checkpoint_dir=None, sample_dir=None, loss_metric = 'L1', flip = False):
        ...
        self.dir_name = "%s-img_sz_%s-fltr_dim_%d-%s" % (
                    self.dataset_name, 
                    self.image_size,
                    self.fcn_filter_dim,
                    self.loss_metric,
    ...
    ###  define graphs
        ...
        self.Ad_logits_fake = self.A_d_net(self.A2B, self.real_A, reuse = False)
        self.Ad_logits_real = self.A_d_net(self.real_B, self.real_A, reuse = True)
        ...
        # self.Bd_logits_fake = self.B_d_net(self.B2A, self.real_B, reuse=False)
        # self.Bd_logits_real = self.B_d_net(self.real_A, self.real_B, reuse=True)
        ...
        # self.d_loss = self.Ad_loss + self.Ag_loss
        # self.g_loss = self.Ag_loss + self.Bg_loss
        self.g_loss = self.Ag_loss
        self.d_loss = self.Ad_loss
    ...
    def load_random_samples(self):
        #np.random.choice(
        slice = random.sample(glob(('./datasets/{}/val/A/*.'+self.suffix).format(self.dataset_name)), 1)
        sample_files_A = glob(slice[0])
        sample_A_imgs = [load_data(f, image_size =self.image_size, flip = False) for f in sample_files_A]

        for f in xrange(len(slice)):
            sample_files_B = []
            sample_files_B.append(glob(slice[f].replace('A','B'))[0])
        sample_B_imgs = [load_data(f, image_size =self.image_size, flip = False) for f in sample_files_B]
    ...
    def sample_shotcut(self, sample_dir, epoch_idx, batch_idx):
        ...
        # Ad, Ag, A2B_imgs = self.sess.run([self.Ad_loss, self.Ag_loss, self.A2B], feed_dict={self.real_A: sample_A_imgs, self.real_B: sample_B_imgs})
        # Bd, Bg, B2A_imgs = self.sess.run([self.Bd_loss, self.Bg_loss, self.B2A], feed_dict={self.real_A: sample_A_imgs, self.real_B: sample_B_imgs})
        # print("[Sample] A2B_d_loss: {:.8f}, A2B_g_loss: {:.8f}, d_loss: {:.8f}, B2A_B_loss: {:.8f}".format(Ad, Ag, Bd, Bg))
        # save_images(B2A_imgs, [self.batch_size,1], './{}/{}/{:06d}_{:04d}_B2A.jpg'.format(sample_dir,self.dir_name, epoch_idx, batch_idx))


        Ad, Ag, A2B_imgs = self.sess.run([self.Ad_loss, self.Ag_loss, self.A2B],
                                         feed_dict={self.real_A: sample_A_imgs, self.real_B: sample_B_imgs})
        print("[Sample] A2B_d_loss: {:.8f}, A2B_g_loss: {:.8f}".format(Ad, Ag))

        save_images(A2B_imgs, [self.batch_size,1], ('./{}/{}/{:06d}_{:04d}_A2B.'+self.suffix).format(sample_dir,self.dir_name , epoch_idx, batch_idx))
    ...
    def train(self, args):
        ...
        data_A_list = data_A
        orig_B = glob(('./datasets/{}/train/B/*.' + self.suffix).format(self.dataset_name))
        data_B = []
        for f in xrange(len(data_A_list)):
            if data_A_list[f].replace('A', 'B') in orig_B:
                data_B.append(data_A_list[f].replace('A', 'B'))
            else:
                del data_B[f]
                del data_A[f]
    ...
    #     _, Adfake,Adreal, Ad, Bd= self.sess.run(
    #         [self.d_optim, self.Ad_loss_fake, self.Ad_loss_real, self.Ad_loss, self.Bd_loss],
    #         feed_dict = {self.real_A: batch_A_imgs, self.real_B: batch_B_imgs})
    #     _, Ag, Bg = self.sess.run(
    #         [self.g_optim, self.Ag_loss, self.Bg_loss],
    #         feed_dict={ self.real_A: batch_A_imgs, self.real_B: batch_B_imgs})
    #
    #     _, Ag, Bg = self.sess.run(
    #         [self.g_optim, self.Ag_loss, self.Bg_loss],
    #         feed_dict={ self.real_A: batch_A_imgs, self.real_B: batch_B_imgs})
    #
    #     print("time: %4.4f, Ad: %.2f, Ag: %.2f, Bd: %.2f, Bg: %.2f" % (time.time() - start_time, Ad,Ag,Bd,Bg))
    #     print("Ad_fake: %.2f, Ad_real: %.2f" % (Adfake,Adreal))

    def run_optim(self,batch_A_imgs, batch_B_imgs,  counter, start_time):
        _, Adfake,Adreal, Ad= self.sess.run(
            [self.d_optim, self.Ad_loss_fake, self.Ad_loss_real, self.Ad_loss],
            feed_dict = {self.real_A: batch_A_imgs, self.real_B: batch_B_imgs})
        _, Ag = self.sess.run(
            [self.g_optim, self.Ag_loss],
            feed_dict={ self.real_A: batch_A_imgs, self.real_B: batch_B_imgs})
        ...
        print("time: %4.4f, Ad: %.2f, Ag: %.2f" % (time.time() - start_time, Ad,Ag))
        print("Ad_fake: %.2f, Ad_real: %.2f" % (Adfake,Adreal))
    ...
    def test_domain(self, args, test_log, type = 'A'):
        ...
        if type == 'A':
            for i in xrange(0, len(test_files)//self.batch_size):
                ...
                print(A_imgs.shape)
                Ag,A2B_imgs = self.sess.run([self.Ag_loss,self.A2B], feed_dict={self.real_A: A_imgs})
                print("Ag_loss: %d"%Ag)
                save_images(A_imgs, [self.batch_size, 1], test_path+filename_o+'_realA.' + self.suffix)
                save_images(A2B_imgs, [self.batch_size, 1], test_path+filename_o+'_A2B.' + self.suffix)
        # elif type=='B':
        #     for i in xrange(0, len(test_files)//self.batch_size):
        #         filename_o = test_files[i*self.batch_size].split('/')[-1].split('.')[0]
        #         idx = i+1
        ...
        #         Ag,B2A_imgs = self.sess.run([self.Ag_loss,self.B2A], feed_dict={self.real_B:B_imgs})
